...
export default function CheckoutProductCard({ product, productIndex, deliveryOption}: ICheckoutProductCard) {
  const dispatch = useStoreDispatch();
  const { productInfo } = useStoreSelector((state) => state.checkout);

  useEffect(() => {
    if (product.uuid) {
      dispatch(productDetailCardThunk({uuid: product.uuid }));
  ...
  console.log("Product in CheckoutProductCard:", product);

  return (
    <>
      {productInfo.map((item) => (
        <div key={item.uuid}  className="grid grid-cols-1 grid-rows-2 md:grid-rows-1 md:grid-cols-[auto,1fr] items-center gap-3 p-3 ">
